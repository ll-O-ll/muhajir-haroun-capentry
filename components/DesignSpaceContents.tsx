...
const DEFAULT_STATEMENT = "We believe that architecture and interior design are most powerful when they are in constant conversation. Our approach to 'Design Space' is rooted in spatial storytelling—creating environments that are not only functional but deeply resonant with the people who inhabit them.";

export default function DesignSpaceContents() {
    const [activeTab, setActiveTab] = useState<"statement" | "projects" | "procedure">("projects");
        ...
        async function fetchData() {
            try {
                const [allProjects, dbProcedures, dbStatement] = await Promise.all([
                    getProjectsAction(),
                    getProcedureStepsAction('design-space'),
                    getDesignStatementAction('design-space')
                ]);

                // Filter only projects belonging to Design Space
                const filtered = allProjects.filter((p: any) => p.category === 'design-space' || !p.category);
                ...
                console.error("Error fetching design space data:", error);
            } finally {
                setLoading(false);
            }
        }
                                    ...
                                    return (
                                        <div key={project.id} className="group cursor-pointer space-y-4">
                                            ...
                                            <div className="space-y-1">
                                                <p className="text-[10px] uppercase tracking-[0.2em] text-muted-foreground">{project.category || "Design Space"}</p>
